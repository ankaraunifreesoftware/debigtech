...
const BUILD_SECTION = {
    ...
    webBasedProducts: () => generateCategorySection('Web-bazlı servisler', readYaml()['web based products']),
    operatingSystems: () => generateCategorySection('İşletim sistemleri', readYaml()['operating systems']),
    desktopApps: () => generateCategorySection('Bilgisayar uygulamaları', readYaml()['desktop applications']),
    mobileApps: () => generateCategorySection('Mobil uygulamalar', readYaml()['mobile applications']),
    hardware: () => generateCategorySection('Donanım', readYaml()['hardware']),
    useful: () => '# Kullanışlı bağlantılar, araçlar ve tavsiyeler',
    resources: () => readFile('md/_resources.md'),
    books: () => generatePublications('Kitaplar', 'books'),
    blogs: () => generatePublications('Blog gönderileri', 'blogs'),
    news: () => generatePublications('Haberler', 'news'),
